...
- Added an opt-in managed-sandbox acceptance test that seeds a canary only in a disposable observer's MemFS, proves the sandboxed Read tool retrieves it, and verifies `send_whisper` executes in the broker process.

### Changed

- Replaced the Claude-specific worker with one harness-neutral broker for Claude Code, Codex, Letta Code, Hermes, and OpenCode.
- Added a real Codex 0.149.1 suite covering nearest-project discovery, prompt and tool observations, prompt and tool-boundary passive delivery, rollout-transcript context, and cross-session isolation. Live testing found that Codex silently discards bare stdout at prompt boundaries, so the Codex adapter now uses `hookSpecificOutput.additionalContext` on `SessionStart` and `UserPromptSubmit` as well as tool events.
- Added opt-in Letta Code 0.30.32 blocker tests instead of claiming unverified passive acceptance. Headless turns do not fire `SessionStart` or `UserPromptSubmit`; an isolated PTY fires `SessionStart`, but its composed prompt is restored unsent before `UserPromptSubmit`. A local App Server persists a queued canary and emits `turn_finished`, but Agent SDK 0.7.6 waits for absent `usage_statistics`. Queued-message drains now time out after five minutes instead of pinning the broker, then reconcile the persisted OTID on the next drain without sending a duplicate turn.
- Added a positive Letta Cloud `queue_message` acceptance test. A disposable Cloud agent and two real conversations prove the broker receives a terminal result, acknowledges the target delivery with `nativeReceipt`, gets model read-back of the canary, and never writes it into the other conversation.
- Scoped Letta Code routes by agent and conversation together. Local 0.30.32 calls the first conversation of every agent `default`; conversation-only route keys could merge two agents in one project and cross their whispers or queued-message identity. Existing stored Letta Code route keys are not migrated, so pending deliveries on the old key expire by TTL.
