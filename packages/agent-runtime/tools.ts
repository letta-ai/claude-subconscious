...
function deliveryTool(
  ...
  return {
    ...
    description: whisper
      ? "Send concise passive context. It arrives at a later supported harness boundary, never the one being observed, which may be later in the current turn or on the next turn. Use only when it will still change the next step on arrival."
